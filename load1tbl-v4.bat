...
@tr < %FPATH%\dump_%DB%\%TBL%.csv -d '\000' | sed "/^$/d;/^$/d;s/\x0//g;s/^/\\N/;s/\\/\\\\/g;s//\\N/g;s//\\N/g;s/$/\\N/;" > %FPATH%\dump_%DB%\%TBL%.tmp
@REM tr < %FPATH%\dump_%DB%\%TBL%.csv -d '\000' | sed "/^$/d;/^$/d;s/\x0//g;s/^/\\N/;s/\\/\\\\/g;s//\\N/g;s//\\N/g;s/$/\\N/;" > %FPATH%\dump_%DB%\%TBL%.tmp
@@REM tr < %FPATH%\dump_%DB%\%TBL%.csv -d '\000' | sed "/^$/d;/^$/d;s/\x0//g;s/^/\\N/;s/\\/\\\\/g;s//\\N/g;s//\\N/g;s/$/\\N/;" > %FPATH%\dump_%DB%\%TBL%.tmp
@REM tr < %FPATH%\dump_%DB%\%TBL%.csv -d '\000' | sed "/^$/d;/^$/d;s/\\x00//g;s/^/\\N/;s/\\/\\\\/g;s//\\N/g;s//\\N/g;s/$/\\N/;" > %FPATH%\dump_%DB%\%TBL%.tmp
